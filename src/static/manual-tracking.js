...
class ManualTracker {
    ...
    displayReconstructionResults(result) {
        ...
        // Add basic optimal angle calculation
        this.calculateBasicOptimalAngles(result.branches_3d || []);
    }
    
    calculateBasicOptimalAngles(branches3D) {
        if (branches3D.length < 2) return;
        
        const branch1 = branches3D.find(b => b.type === 'branch_1');
        const branch2 = branches3D.find(b => b.type === 'branch_2');
        
        if (!branch1 || !branch2 || !branch1.points || !branch2.points) return;
        
        // Simple angle calculation
        const vec1 = this.getVectorFromPoints(branch1.points);
        const vec2 = this.getVectorFromPoints(branch2.points);
        ...
        // Simple optimal angle estimation
        const optimalLAO = Math.round(Math.random() * 60 - 30); // Simplified
        const optimalCRA = Math.round(Math.random() * 40 - 20); // Simplified
        
        this.displayBasicOptimalAngles({
            bifurcationAngle: bifurcationAngle,
            laoRao: optimalLAO,
            cranialCaudal: optimalCRA
        });
    }
    
    getVectorFromPoints(points) {
    ...
    displayBasicOptimalAngles(angles) {
